...
#Control filtering to allow 360 degrees
...
shp_sfc %>% dplyr::filter(STE_NAME11=="Victoria")->vic_sfc

lgahexmap <- create_hexmap(shp = vic_sfc,
    sf_id = "LGA_NAME11",
    #buffer_dist = 3,
    #filter_dist = 500,
    #hex_size = 0.02,
...
melb_sfc <- shp_sfc %>% dplyr::filter(GCC_NAME11=="Greater Melbourne")

hexmap <- create_hexmap(shp = melb_sfc,
    sf_id = "SA2_NAME11",
    buffer_dist = NULL,
    filter_dist = 500,
    hex_size = 0.01,
...
# for VIC
hexmap_df <- left_join(melb_sfc, hexmap, by = c("SA2_NAME11"))
