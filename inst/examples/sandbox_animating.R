#animate between hex and polygons

#Merge the hex and polygon sets
...
hex_points_df <- hexmap_df %>%
    split(.[[sf_id]]) %>%
    map_dfr(hex_points)

 system.time(
    hex_points_df <- full_join(hexmap_df,
        hex_points_df,
        by = c("LGA_NAME11")))

# join two sets to plot using animation

fortified_qld_3 <- fortified_qld_3 %>% rename(plot_long = long,
    plot_lat = lat)

hexmap_qld_3 <- hexmap_qld_3 %>% rename(hex_long = long,
    hex_lat = lat)
...
ggplot(qld_3) + geom_polygon(aes(x=hex_long, y=hex_lat, group = SA2_NAME11))
