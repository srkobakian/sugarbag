find_lat_group <- function(data, hull_points, buffer_dist) {
    ...
    if (NROW(lat_hull) > 0) {
        ...
        min_long <- lat_hull[which.min(lat_hull$latitude),] %>% mutate(new_grad = ifelse(abs(gradient) < 1, 1, gradient),
            cut_long =  longitude - ((latitude - hex_lat)/new_grad) - buffer_dist) %>%
            pull(cut_long)
        # algebra to find lat for maximum
        max_long <- lat_hull[which.max(lat_hull$latitude),] %>% mutate(new_grad = ifelse(abs(gradient) < 1, 1, gradient),
            cut_long =  longitude - ((latitude - hex_lat)/new_grad) + buffer_dist) %>%
            pull(cut_long)
