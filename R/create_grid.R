...
create_grid <- function(bbox, radius, expand_grid = 0.1) {

    expand <- (bbox$max[1] - bbox$min[1]) * expand_grid
    grid <- tibble::as.tibble(expand.grid(hex_long = seq(bbox$min[1] - expand,
                                                 bbox$max[1] + expand,
                                                 radius),
                                  hex_lat = seq(bbox$min[2] - expand,
                                                bbox$max[2] + expand,
                                                radius)))
